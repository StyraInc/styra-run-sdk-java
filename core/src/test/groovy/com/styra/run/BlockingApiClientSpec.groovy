...
class BlockingApiClientSpec extends Specification {
    def "DefaultApiClient is used by default when no factory is specified"() {
        ...
        client instanceof BlockingApiClient
    }
    ...
    def "API client can do basic requests"(Method method, int status, String path, Map<String, String> headers, String requestBody, String responseBody) {
        ...
        and: 'an API client'
        def client = new BlockingApiClient(new ApiClient.Config(SSLContext.getDefault(),
...
//    @Ignore("Fails with TimeoutException on GitHub")
    ...
    def "Configured request timeout is respected (#requestTimeout)"() {
        ...
        e.cause.cause instanceof SocketTimeoutException
        e.cause.cause.message == 'Read timed out'
    ...
    def "Configured SSL Context is respected"() {
        ...
        def clientWithStandardTrust = new BlockingApiClient(new ApiClient.Config(SSLContext.getDefault(),
                Duration.ofSeconds(2), Duration.ofSeconds(2), "test"))
        ...
        def clientWithCustomTrust = new BlockingApiClient(new ApiClient.Config(customSslContext,
                Duration.ofSeconds(2), Duration.ofSeconds(2), "test"))
